...
class Format
{
    ...
    /**
    * Get Sales Layer format params data.
    ...
    */
    public function getProductFormatParamsToStore($sl_language)
    {
        ...
        if ($this->debug_level > 1 &&
            ...
            sl_debug("Format additional fields: ".print_r($format_params['format_additional_fields'], true));
    ...
    /**
    * Prepare Sales Layer product formats data.
    ...
    */
    public function prepareProductFormatDataToStore($formats)
    {
        ...
        $time_ini_pre_process_formats = microtime(true);
        ...
        sl_debug('### pre_process_formats: '.(microtime(true) - $time_ini_pre_process_formats).' seconds.');

        $time_ini_reorganize_formats = microtime(true);
        $formats = $this->reorganize_formats_before_store($formats);
        sl_debug('### reorganize_formats: '.(microtime(true) - $time_ini_reorganize_formats).' seconds.');
            ...
            if ($format_ok){
                ...
                if (!empty($attribute_data_empty)){
                    ...
                    foreach ($attribute_data_empty as $error_msg) {
                            ...
                            $error_message .= "\r\n".$format_data[$this->format_field_sku]." - ".print_r($error_msg, true);
    ...
    /**
     * Synchronize a stored product format.
    ...
    public function sync_stored_product_format($format)
    {
        ...
        if (!$wp_parent_product){
            ...
            // $time_ini_get_parent_attributes = microtime(true);
            $parent_attributes = $this->get_parent_attributes($wp_parent_product['ID'], $format['sl_attributes']);
            // sl_debug('## time_get_parent_attributes: '.(microtime(true) - $time_ini_get_parent_attributes).' seconds.', 'timer');
            
        }
        
        // $time_ini_sync_parent_data = microtime(true);
        $this->sync_parent_data($wp_parent_product['ID'], $parent_attributes);
        // sl_debug('## time_sync_parent_data: '.(microtime(true) - $time_ini_sync_parent_data).' seconds.', 'timer');

        $time_ini_format_core_data = microtime(true);
        ...
        if (isset($format_data[$this->format_field_shipping_class])){
            ...
            if (taxonomy_exists('product_shipping_class')){
                ...
                if (is_object($wp_format_shipping_class_term)){
                    ...
                    } elseif (is_array($wp_format_shipping_class_term) && isset($wp_format_shipping_class_term[0]) && isset($wp_format_shipping_class_term[0]['term_id'])){
            	...
            	if (is_array($format_data[$this->format_field_shipping_class]) && !empty($format_data[$this->format_field_shipping_class])){
                    ...
                    } elseif (!is_array($format_data[$this->format_field_shipping_class]) && $format_data[$this->format_field_shipping_class] != ''){
        		...
        		if ($wp_format_shipping_class != $sl_id_format_shipping_class){
                	...
                	if ($sl_id_format_shipping_class == '' && $wp_format_shipping_class != ''){
                        ...
                        } elseif ($sl_id_format_shipping_class != ''){
        ...
        if ($prices_updated){
            ...
            $time_ini_parent_price = microtime(true);
            ...
            sl_debug('## time_parent_price: '.(microtime(true) - $time_ini_parent_price).' seconds.', 'timer');
        ...
        if (isset($format_data[$this->format_field_manage_stock])){
            ...
            if (is_array($sl_format_manage_stock) && !empty($sl_format_manage_stock)){
                ...
                } elseif (!is_array($sl_format_manage_stock) && $sl_format_manage_stock !== '' && !is_bool($sl_format_manage_stock)){
                ...
                if ($sl_manage_stock == 'no'){
                    ...
                    if (isset($format_data[$this->format_field_stock_status])){
                        ...
                        if (is_array($sl_format_stock_status) && !empty($sl_format_stock_status)){
                            ...
                            } elseif (!is_array($sl_format_stock_status) && $sl_format_stock_status !== ''){
                ...
                if (is_numeric($sl_stock) && (!$sl_stock_not_num || ($sl_stock_not_num && $sl_stock !== 0))){
                    ...
                    if (!isset($wp_format['_stock']) || (isset($wp_format['_stock']) && $wp_format['_stock'] != $sl_stock)){
                        ...
                        if ((!isset($wp_format['_stock_status']) || (isset($wp_format['_stock_status']) && $wp_format['_stock_status'] == 'outofstock')) && $sl_stock > 0){
                            ...
                            } elseif ((!isset($wp_format['_stock_status']) || (isset($wp_format['_stock_status']) && $wp_format['_stock_status'] == 'instock')) && $sl_stock <= 0){
        ...
        sl_debug('## time_format_core_data: '.(microtime(true) - $time_ini_format_core_data).' seconds.', 'timer');
                
        $time_ini_format_images = microtime(true);
        //Format images
        ...
        sl_debug('## time_format_images: '.(microtime(true) - $time_ini_format_images).' seconds.', 'timer');
    ...
    /**
     * Get a product parent attributes.
    ...
    public function find_format_by_attributes($product_id, $comp_id, $format_id, $format_attributes)
    {
        ...
        if( is_wp_error( $posts ) ) {
            ...
            if (in_array($wp_saleslayerid, array(0,'',null)) && in_array($wp_saleslayercompid, array(0,'',null)) && in_array($wp_saleslayerformatid, array(0,'',null))){
                ...
                } elseif ($wp_saleslayerid == $product_id && $wp_saleslayercompid == $comp_id && $wp_saleslayerformatid == $format_id){
    ...
    /**
     * Synchronize variable-parent attributes for a product.
     *
     * @param int   $wp_parent_product_id Parent product ID.
     * @param array $sl_parent_attributes Parent attributes keyed by attribute name.
     * @return void
     */
    ...
    /**
     * Delete a stored product format.
