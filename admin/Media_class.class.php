<?php

require_once(ABSPATH . 'wp-load.php');
require_once(ABSPATH . 'wp-admin/includes/image.php');
include_once(SLYR_WC__PLUGIN_DIR.'admin/general_functions.php');

/**
 * Media service.
 *
 * Handles downloading, creating and updating WordPress media attachments
 * used by the Sales Layer WooCommerce integration.
 */
class Media_class
{

    private static $instance;

    protected	$stored_url_files_sizes             = array();
    protected	$sl_time_ini_media_meta_process;
    protected	$max_media_meta_execution_time		= 240;
    protected	$end_media_meta_process;

    /**
     * Get singleton instance.
     * @return Media_class Singleton instance
     */
    public static function &get_instance()
    {

        if ( is_null( self::$instance ) ) {
            
            self::$instance = new Media_class();
        
        }
        
        return self::$instance;
    
    }

    /**
     * Get filename from an image URL.
     * @param string $image_url URL
     * @return string File name derived from URL
     */
    public function get_image_url_filename($image_url)
    {

        // $time_ini_get_image_url_filename = microtime(true);

        $image_url_info = pathinfo($image_url);
        $filename = rawurldecode($image_url_info['basename']);
        $filename = str_replace(["#", "%"], ["_U23_", "_U25_"], $filename);

        // sl_debug('# time_get_image_url_filename: '.(microtime(true) - $time_ini_get_image_url_filename).' seconds.', 'timer');

        return $filename;

    }

    /**
     * Get encoded basename url
     * @param  string $url 		url to encode
     * @return string      		url with encoded basename
     */
    protected function get_encoded_url($url)
    {

        // $time_ini_get_encoded_url = microtime(true);

        $image_url_info = pathinfo($url);
        $filename_encoded = $image_url_info['basename'];            
        $filename_encoded = rawurlencode(rawurldecode($filename_encoded));
        
        // sl_debug('# time_get_encoded_url: '.(microtime(true) - $time_ini_get_encoded_url).' seconds.', 'timer');

        return $image_url_info['dirname'].'/'.$filename_encoded;

    }

    /**
     * Download media and create an attachment for a post.
     * @param string $file_url URL of the file to fetch
     * @param int    $post_id  Post ID to assign the file
     * @param bool   $process_meta Whether to generate and persist metadata now
     * @return int|false Attachment ID on success, false on failure
     */
    public function fetch_media($file_url, $post_id, bool $process_meta = false)
    {

        set_time_limit(60);

        $time_ini_fetch_media = microtime(true);
        
        // $time_ini_new_filename = microtime(true);

        $new_filename = $this->get_image_url_filename($file_url);

        if (!$new_filename || !$post_id){
            
            return false;
        
        }

        // sl_debug('# fetch_media() - time_new_filename: '.(microtime(true) - $time_ini_new_filename).' seconds.', 'timer');

        sl_debug(" > Importing media: ".$file_url.($process_meta ? ' - Processing Meta' : ''));

        //directory to import to	
        $artDir = 'wp-content/uploads/importedmedia/';
        
        // $time_ini_mkdir = microtime(true);
        //if the directory doesn't exist, create it	
        if(!file_exists(ABSPATH.$artDir)) {
            mkdir(ABSPATH.$artDir, 0755, true);
        }
        // sl_debug('# fetch_media() - time_mkdir: '.(microtime(true) - $time_ini_mkdir).' seconds.', 'timer');

        $file_url_encoded = $this->get_encoded_url($file_url);

        $temp_file = download_url($file_url_encoded);
        if (is_wp_error($temp_file)) {
            sl_debug('## Error. Downloading file: '.$file_url_encoded);
            return false;
        }
        rename($temp_file, ABSPATH.$artDir.$new_filename);
        
        $siteurl = get_option('siteurl');
        $file_info = getimagesize(ABSPATH.$artDir.$new_filename);

        //create an array of attachment data to insert into wp_posts table
        $artdata = array(
            'post_author' => 1, 
            'post_date' => current_time('mysql'),
            'post_date_gmt' => current_time('mysql'),
            'post_title' => $new_filename, 
            'post_status' => 'inherit',
            'comment_status' => 'closed',
            'ping_status' => 'closed',
            'post_name' => sanitize_title_with_dashes(str_replace("_", "-", $new_filename)),
            'post_modified' => current_time('mysql'),
            'post_modified_gmt' => current_time('mysql'),
            'post_parent' => $post_id,
            'post_type' => 'attachment',
            'guid' => $siteurl.'/'.$artDir.$new_filename,
            'post_mime_type' => $file_info['mime'],
            'post_excerpt' => '',
            'post_content' => ''
        );

        $uploads = wp_upload_dir();
        $save_path = $uploads['basedir'].'/importedmedia/'.$new_filename;
        // sl_debug('# fetch_media() - time_prepare_data: '.(microtime(true) - $time_ini_prepare_data).' seconds.', 'timer');

        // $time_ini_insert_attachment = microtime(true);
        //insert the database record
        $attach_id = wp_insert_attachment( $artdata, $save_path, $post_id );
        // sl_debug('# fetch_media() - time_insert_attachment: '.(microtime(true) - $time_ini_insert_attachment).' seconds.', 'timer');

        if ( !is_wp_error($attach_id) ) {

            if (!$process_meta){
                
                $meta_required_data = array('save_path' => rawurlencode($save_path), 'n_try' => 0);
                $result_add = add_post_meta( $attach_id, '_meta_required', json_encode($meta_required_data));
                
                if (!$result_add){

                    sl_debug('## Error. Adding media post meta: '.$save_path);
                    return false;

                }

            }else{

                // $time_ini_generate_attachment_metadata = microtime(true);

                if ($attach_data = wp_generate_attachment_metadata( $attach_id, $save_path)) {

                    // sl_debug('# fetch_media() - time_generate_attachment_metadata: '.(microtime(true) - $time_ini_generate_attachment_metadata).' seconds.', 'timer');
                    // $time_ini_update_attachment_metadata = microtime(true);
                    wp_update_attachment_metadata($attach_id, $attach_data);
                    // sl_debug('# fetch_media() - time_update_attachment_metadata: '.(microtime(true) - $time_ini_update_attachment_metadata).' seconds.', 'timer');
                
                }else{

                    // sl_debug('# update_media() - time_generate_attachment_metadata: '.(microtime(true) - $time_ini_generate_attachment_metadata).' seconds.', 'timer');
            		sl_debug('## Error. Generating attachment metadata for file: '.$save_path);

            	}
            	
            }

        }else{

            sl_debug('## Error. Inserting new media attachment: '.$save_path);
            return false;

        }

        sl_debug('# time_fetch_media: '.(microtime(true) - $time_ini_fetch_media).' seconds.', 'timer');
        return $attach_id;

    }

    /**
     * Update media item from a URL.
     * @param  string $file_url      		media url to update
     * @param  int $attachment_id 			id media item to update
     * @param  boolean $process_meta  		process media or store for media process cron
     * @return boolean						result of update
     */
    public function update_media($file_url, $attachment_id, bool $process_meta = false)
    {

        $time_ini_update_media = microtime(true);
        
        $new_filename = $this->get_image_url_filename($file_url);

        if (!$new_filename || !$attachment_id){
            
            return false;
        
        }

        sl_debug(" > Updating media: ".$file_url.($process_meta ? ' - Processing Meta' : ''));

        $uploads_path = 'wp-content/uploads/';
        $importedmedia_path = $uploads_path.'importedmedia/';

        // $time_ini_check_existing_image = microtime(true);

        $existing_attachment_metadata = wp_get_attachment_metadata($attachment_id);
        
        if (!empty($existing_attachment_metadata)){

            $main_file_to_delete_path = ABSPATH.$uploads_path.$existing_attachment_metadata['file'];

            try{
                
                @unlink($main_file_to_delete_path);
                
            }catch(\Exception $e){
            
            	sl_debug('## Error. Deleting main media '.$main_file_to_delete_path.': '.$e->getMessage());
            
            }

            if (isset($existing_attachment_metadata['sizes']) && !empty($existing_attachment_metadata['sizes'])){
           
                foreach ($existing_attachment_metadata['sizes'] as $keySize => $meta_size){
                	
                	$file_to_delete_path = ABSPATH.$importedmedia_path.$meta_size['file'];

                	try{
                        
                        @unlink($file_to_delete_path);

                	}catch(\Exception $e){
                    
                    	sl_debug('## Error. Deleting media '.$file_to_delete_path.': '.$e->getMessage());
                    
                    }

                }
                
            }

        }
        // sl_debug('# update_media() - time_check_existing_image: '.(microtime(true) - $time_ini_check_existing_image).' seconds.', 'timer');

        try{

            // $time_ini_mkdir = microtime(true);
            //if the directory doesn't exist, create it 
            if (!file_exists(ABSPATH.$importedmedia_path)) {

                mkdir(ABSPATH.$importedmedia_path);

            }
            // sl_debug('# update_media() - time_mkdir: '.(microtime(true) - $time_ini_mkdir).' seconds.', 'timer');

            $file_url_encoded = $this->get_encoded_url($file_url);
            
            // $time_ini_copy_image = microtime(true);
            copy($file_url_encoded, ABSPATH.$importedmedia_path.$new_filename);
            // sl_debug('# update_media() - time_copy_image: '.(microtime(true) - $time_ini_copy_image).' seconds.', 'timer');
            
            // $time_ini_update_post = microtime(true);

            $siteurl = get_option('siteurl');
            $file_info = getimagesize(ABSPATH.$importedmedia_path.$new_filename);

            $post_data = array(
             	'ID' => $attachment_id,
             	'post_title' => $new_filename,
             	'post_name' => sanitize_title_with_dashes(str_replace("_", "-", $new_filename)),
             	'guid' => $siteurl.'/'.$importedmedia_path.$new_filename,
             	'post_mime_type' => $file_info['mime']
            );

            wp_update_post($post_data);
            // sl_debug('# update_media() - time_update_post: '.(microtime(true) - $time_ini_update_post).' seconds.', 'timer');

            $save_path = ABSPATH.$importedmedia_path.$new_filename;

            if (!$process_meta){

            	$meta_required_data = array('save_path' => rawurlencode($save_path), 'n_try' => 0);
            	$result_add = add_post_meta( $attachment_id, '_meta_required', json_encode($meta_required_data));
            	
            	if (!$result_add){

            		sl_debug('## Error. Adding media post meta: '.$save_path);
            		return false;

            	}

            }else{

                // $time_ini_generate_attachment_metadata = microtime(true);
            		
            	if ($attach_data = wp_generate_attachment_metadata( $attachment_id, $save_path)) {
            	    
                    // sl_debug('# update_media() - time_generate_attachment_metadata: '.(microtime(true) - $time_ini_generate_attachment_metadata).' seconds.', 'timer');

                    // $time_ini_update_attachment_metadata = microtime(true);

            	    wp_update_attachment_metadata($attachment_id, $attach_data);
            	
                    // sl_debug('# update_media() - time_update_attachment_metadata: '.(microtime(true) - $time_ini_update_attachment_metadata).' seconds.', 'timer');

            	}else{

                    // sl_debug('# update_media() - time_generate_attachment_metadata: '.(microtime(true) - $time_ini_generate_attachment_metadata).' seconds.', 'timer');
            		sl_debug('## Error. Generating attachment metadata for file: '.$save_path);

            	}

            }

            // $time_ini_update_attached_file = microtime(true);
            update_attached_file($attachment_id, $save_path);
            // sl_debug('# update_media() - time_update_attached_file: '.(microtime(true) - $time_ini_update_attached_file).' seconds.', 'timer');

        }catch(\Exception $e){

            sl_debug('## Error. Updating media: '.$e->getMessage());
            return false;

        }

        sl_debug('# time_update_media: '.(microtime(true) - $time_ini_update_media).' seconds.', 'timer');
        return $attachment_id;

    }

    /**
     * Delete a media attachment and its files.
     * @param int $attachment_id Attachment ID to delete
     * @return bool True on success, false on failure
     */
    public function delete_media($attachment_id)
    {
        
        $time_ini_delete_media = microtime(true);

        sl_debug(" > Deleting media from attachment_id: $attachment_id");
        
        $uploads_path = 'wp-content/uploads/';
        $importedmedia_path = $uploads_path.'importedmedia/';

            try{

                $termmeta_count = sl_connection_query(
                    'read',
                    " SELECT count(*) as sl_cuenta_registros FROM ".WPDB_PREFIX."termmeta WHERE meta_key = %s AND meta_value = %d",
                    ['thumbnail_id', (int)$attachment_id]
                );
            
            }catch(\Exception $e){

            sl_debug('## Error. Reading termmeta_count: '.$e->getMessage());

        }

        if (isset($termmeta_count['sl_cuenta_registros']) && $termmeta_count['sl_cuenta_registros'] > 0){
        
            return false;
        
        }

            try{

                $postmeta_thumbnail_count = sl_connection_query(
                    'read',
                    " SELECT count(*) as sl_cuenta_registros FROM ".WPDB_PREFIX."postmeta WHERE meta_key = %s AND meta_value = %d",
                    ['_thumbnail_id', (int)$attachment_id]
                );
            
            }catch(\Exception $e){

            sl_debug('## Error. Reading postmeta_thumbnail_count: '.$e->getMessage());

        }

        if (isset($postmeta_thumbnail_count['sl_cuenta_registros']) && $postmeta_thumbnail_count['sl_cuenta_registros'] > 0){
        
            return false;
        
        }

        try{

            $postmeta_image_galleries = sl_connection_query(
                'read',
                " SELECT * FROM ".WPDB_PREFIX."postmeta WHERE meta_key = %s AND meta_value LIKE %s",
                ['_product_image_gallery', '%'.(string)$attachment_id.'%']
            );

        }catch(\Exception $e){

            sl_debug('## Error. Reading postmeta_image_galleries: '.$e->getMessage());

        }

        if (!empty($postmeta_image_galleries)){

            foreach ($postmeta_image_galleries as $posmeta_id => $postmeta_image_gallery){

                if (strpos($postmeta_image_gallery['meta_value'], ',')){

                    $postmeta_image_gallery = explode(',', $postmeta_image_gallery['meta_value']);

                    foreach ($postmeta_image_gallery as $image_id) {

                       if ($image_id == $attachment_id){

                          return false;

                      }

                    }

                }else{

                    if ($postmeta_image_gallery['meta_value'] == $attachment_id){

                        return false;

                    }

                }

            }

        }

        try{

            wp_delete_attachment( $attachment_id );

        }catch(\Exception $e){

            sl_debug('## Error. Deleting attachment: '.$e->getMessage());

        }

        $existing_attachment_metadata = wp_get_attachment_metadata($attachment_id);
        
        if (!empty($existing_attachment_metadata)){

            $main_file_to_delete_path = ABSPATH.$uploads_path.$existing_attachment_metadata['file'];

            try{
                
                @unlink($main_file_to_delete_path);
                
            }catch(\Exception $e){
            
            	sl_debug('## Error. Deleting main media '.$main_file_to_delete_path.': '.$e->getMessage());
            
            }

            if (isset($existing_attachment_metadata['sizes']) && !empty($existing_attachment_metadata['sizes'])){
           
                foreach ($existing_attachment_metadata['sizes'] as $keySize => $meta_size){
                	
                	$file_to_delete_path = ABSPATH.$importedmedia_path.$meta_size['file'];

                	try{
                        
                        @unlink($file_to_delete_path);

                	}catch(\Exception $e){
                    
                    	sl_debug('## Error. Deleting media '.$file_to_delete_path.': '.$e->getMessage());
                    
                    }

                }
                
            }

        }

        sl_debug('# time_delete_media: '.(microtime(true) - $time_ini_delete_media).' seconds.', 'timer');
        return true;

    }

    /**
     * Get attachment ID by image title.
     * @param string $thumbnail_title 	thumbnail title
     * @return int Attachment ID or 0 if not found
     */
    public function get_thumbnail_id_by_title($thumbnail_title)
    {
        
        // $time_ini_get_thumbnail_id_by_title = microtime(true);

        $query_thumbnail_args = array(
            'post_type'      => 'attachment',
            'post_mime_type' => 'image',
            'post_status'    => 'inherit',
            'posts_per_page' => - 1,
            'title' 		 => $thumbnail_title,
        );
        
        $query_thumbnail = new WP_Query( $query_thumbnail_args );
        
        if (isset($query_thumbnail->posts[0])){
            
            return $query_thumbnail->posts[0]->ID;

        }

        // sl_debug('# time_get_thumbnail_id_by_title: '.(microtime(true) - $time_ini_get_thumbnail_id_by_title).' seconds.', 'timer');
        return 0;

    }

    /**
     * Read image file size (bytes) from URL.
     * @param string $url Image URL
     * @return int|false File size in bytes or false when unknown
     */
    public function read_image_file_size($url)
    {
        
        if (strpos($url, 'http') !== false){
            
            try{

                // $time_ini_check_url_size = microtime(true);
                
                if (isset($this->stored_url_files_sizes[$url])){

                    // sl_debug('# time_check_size_url stored: '.(microtime(true) - $time_ini_check_url_size).' seconds.', 'timer');
                    return $this->stored_url_files_sizes[$url];

                }else{
                    
                    $image_url_info = pathinfo($url);
                    $filename_encoded = $image_url_info['basename'];	            
                    $filename_encoded = rawurlencode(rawurldecode($filename_encoded));
                    $image_url_encoded = $image_url_info['dirname'].'/'.$filename_encoded;

                    $headers = get_headers($image_url_encoded, TRUE);
                    // sl_debug('# time_check_size_url: '.(microtime(true) - $time_ini_check_url_size).' seconds.', 'timer');

                }

            }catch(\Exception $e){

                sl_debug("## Error. Remote image with URL ".$url." couldn't been synchronized: ".$e->getMessage());
                return false;

            }

            if (isset($headers['Content-Length'])){
               
               $this->stored_url_files_sizes[$url] = $headers['Content-Length'];
               return $headers['Content-Length'];
            
            } elseif (isset($headers['content-length'])){
            
                $this->stored_url_files_sizes[$url] = $headers['content-length'];
                return $headers['content-length'];
            
            }
        
            return false;

        }else{
            
            try{

                // $time_ini_check_local_size = microtime(true);
                $url_filesize = filesize($url);
                clearstatcache();
                // sl_debug('# time_check_size_local: '.(microtime(true) - $time_ini_check_local_size).' seconds.', 'timer');
                return $url_filesize; 

            }catch(\Exception $e){

                sl_debug("## Notice. Could not read local image with URL ".$url." : ".$e->getMessage());

            }
            
            return false;

        }
        
    }

    /**
     * Process pending attachments metadata for deferred media items.
     * @return void
     */
    public function process_pending_meta()
    {
        
        sl_debug("==== Media meta process INIT ".date('Y-m-d H:i:s')." ====", 'mediameta');

        $this->sl_time_ini_media_meta_process = microtime(true);
        $this->end_media_meta_process = false;
        $return_message = '';
        
        $this->check_pending_meta();

        try{

            $sql_meta_count = sl_connection_query('read', " SELECT count(*) as sl_cuenta_registros FROM ".WPDB_PREFIX."postmeta WHERE meta_key = '_meta_required' AND meta_value NOT LIKE '%start_meta_process%' LIMIT 1");
        
        }catch(\Exception $e){

            sl_debug('## Error. Reading sql_meta_count: '.$e->getMessage(), 'mediameta');

        }

        if (isset($sql_meta_count['sl_cuenta_registros']) && $sql_meta_count['sl_cuenta_registros'] > 0){
        
            sl_debug('Pending media meta items to process: '.$sql_meta_count['sl_cuenta_registros'], 'mediameta');

            $sql_meta_required = " SELECT * FROM ".WPDB_PREFIX."postmeta WHERE meta_key = '_meta_required' AND meta_value NOT LIKE '%start_meta_process%' LIMIT 1";

            do{

            	$this->check_media_meta_process_time();

            	if ($this->end_media_meta_process){
            		
            	    sl_debug('Breaking media meta process due to time limit.', 'mediameta');
            	    $return_message = 'Breaking media meta process due to time limit.';
            	    break;

            	}else{

                	$meta_required_to_process = sl_connection_query('read', $sql_meta_required);

                    if ($meta_required_to_process && isset($meta_required_to_process[0])){

                    	$meta_id = $meta_required_to_process[0]['meta_id'];
                        $attach_id = $meta_required_to_process[0]['post_id'];
                    	$meta_data_required_to_process = json_decode(stripslashes($meta_required_to_process[0]['meta_value']), true);
                    	$save_path = rawurldecode($meta_data_required_to_process['save_path']);
                    	$n_try = $meta_data_required_to_process['n_try'];

                    	sl_debug('File to process media meta: '.$save_path.' - Try: '.$n_try, 'mediameta');

                    	$meta_required_data = array('save_path' => rawurlencode($save_path), 'n_try' => $n_try, 'start_meta_process' => strtotime('now'));
                        if (!$result_update = update_metadata_by_mid( 'post', $meta_id, json_encode($meta_required_data))){

                            sl_debug('## Error. Updating start meta process to item with meta_id: '.$meta_id, 'mediameta');
                            
                            $this->sl_delete_metadata_by_mid($meta_id);

                        }else{

                            // $time_ini_generate_attachment_metadata = microtime(true);
                        	if ($attach_data = wp_generate_attachment_metadata( $attach_id, $save_path)) {

                                // sl_debug('# process_pending_meta() - time_generate_attachment_metadata: '.(microtime(true) - $time_ini_generate_attachment_metadata).' seconds.', 'mediameta');
                                // $time_ini_update_attachment_metadata = microtime(true);
                        		wp_update_attachment_metadata($attach_id, $attach_data);
                                // sl_debug('# process_pending_meta() - time_update_attachment_metadata: '.(microtime(true) - $time_ini_update_attachment_metadata).' seconds.', 'mediameta');
                        		$this->sl_delete_metadata_by_mid($meta_id);

                        	}else{

                                // sl_debug('# process_pending_meta() - time_generate_attachment_metadata: '.(microtime(true) - $time_ini_generate_attachment_metadata).' seconds.', 'mediameta');
                        		sl_debug('## Error. Generating attachment metadata for file: '.$save_path, 'mediameta');

                        		$n_try++;

                        		if ($n_try <= 2){

                            		$meta_required_data = array('save_path' => rawurlencode($save_path), 'n_try' => $n_try);
                					if (!$result_update = update_metadata_by_mid( 'post', $meta_id, json_encode($meta_required_data))){
                        			
                						sl_debug('## Error. Updating process tries to item with meta_id: '.$meta_id, 'mediameta');

                						$this->sl_delete_metadata_by_mid($meta_id);

                				    }

                        		}else{

                        			$this->sl_delete_metadata_by_mid($meta_id);

                        		}

                        	}

                        }

                    }

                }

            }while(!empty($meta_required_to_process));

        	if ($return_message == '') $return_message = 'Sales Layer media meta process finished correctly.';

        }else{
            
            sl_debug('There are no pending media meta items to process.', 'mediameta');
            $return_message = 'There are no pending media meta items to process.';
            
        }

        sl_debug('### time_process_pending_meta: '.(microtime(true) - $this->sl_time_ini_media_meta_process).' seconds.', 'mediameta');

        sl_debug("==== Media meta process END ====", 'mediameta');
        
        return $return_message;

    }

    /**
     * Delete metadata by mid
     * @param  int $meta_id 		meta id
     * @return void
     */
    private function sl_delete_metadata_by_mid($meta_id)
    {

        if (!$result_delete = delete_metadata_by_mid( 'post', $meta_id )){

    		sl_debug('## Error. Deleting item with meta_id '.$meta_id, 'mediameta');
            
        }

    }

    /**
     * Check current process time to avoid exceding the limit.
     * @return void
     */
    private function check_media_meta_process_time()
    {

        $current_process_time = microtime(true) - $this->sl_time_ini_media_meta_process;

        if ($current_process_time >= $this->max_media_meta_execution_time){

            $this->end_media_meta_process = true;

        }

    }

    /**
     * Check if there is pending meta to process
     * @return void
     */
    private function check_pending_meta()
    {

        $sql_pending_meta = " SELECT * FROM ".WPDB_PREFIX."postmeta WHERE meta_key = '_meta_required' AND meta_value LIKE '%start_meta_process%'";

        $all_pending_meta = sl_connection_query('read', $sql_pending_meta);
     
        if (!empty($all_pending_meta)){

        	$now = strtotime('now');
    
        	foreach ($all_pending_meta as $pending_meta) {
        			
                $meta_id = $pending_meta['meta_id'];
                $attach_id = $pending_meta['post_id'];
                $pending_meta_data = json_decode(stripslashes($pending_meta['meta_value']), true);
                if (isset($pending_meta_data['start_meta_process'])){

                    $interval  = abs($now - $pending_meta_data['start_meta_process']);
                    $minutes   = round($interval / 60);
                    
                    if ($minutes < 10){
                    
                        sl_debug('Less than 10 minutes processing item with meta_id '.$meta_id.', we let it finish.', 'mediameta');

                    }else{
                            
                        sl_debug('More than 10 minutes processing item with meta_id '.$meta_id.', we set it to run one last time.', 'mediameta');
                    	$meta_required_data = array('save_path' => $pending_meta_data['save_path'], 'n_try' => 3);
                        if (!$result_update = update_metadata_by_mid( 'post', $meta_id, json_encode($meta_required_data))){

                            sl_debug('## Error. Updating last try to pending item with meta_id: '.$meta_id, 'mediameta');

                            if (!$result_delete = $this->sl_delete_metadata_by_mid( 'post', $meta_id )){

                        		sl_debug('## Error. Deleting pending item with meta_id: '.$meta_id, 'mediameta');

                        	}

                        }

                    }

                }

        	}

        }

    }

}
