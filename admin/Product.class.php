...
class Product
{
    ...
    /**
    * Get Sales Layer product params data.
    ...
    */
    public function getProductParamsToStore($sl_language)
    {
        ...
        $product_data_to_store = array();
            ...
            $fixed_product_fields = array(
                ...
                $this->product_field_manage_stock,
                $this->product_field_stock_status,
                ...
                $this->product_field_purchase_note,
                $this->product_field_tags,
                $this->product_field_downloadable,
                $this->product_field_virtual,
                $this->product_field_related_references,
                $this->product_field_crosssell_references,
                $this->product_field_upsell_references,
                $this->product_field_grouping_references,
                $this->product_field_shipping_class
            );
        ...
        if ($this->debug_level > 1 &&
            ...
            sl_debug("Product additional fields: ".print_r($product_params['product_additional_fields'], true));
    ...
    /**
    * Prepare Sales Layer products data.
    ...
    */
    public function prepareProductDataToStore($products, $product_params)
    {
        ...
        $time_ini_pre_process_products = microtime(true);
        ...
        sl_debug('### pre_process_products: '.(microtime(true) - $time_ini_pre_process_products).' seconds.', 'timer');
    ...
    public function sync_stored_product($product)
    {
        ...
        $time_ini_product_core_data = microtime(true);
        ...
        if ($this->debug_level) sl_debug(" > Updating product ID: $sl_product_id (categories: ".print_r($wp_category_ids, true).")");
        ...
        if (isset($product_data[$this->product_field_shipping_class])){
            ...
            if (taxonomy_exists('product_shipping_class')){
                ...
                if (is_object($wp_product_shipping_class_term)){
                    ...
                    } elseif (is_array($wp_product_shipping_class_term) && isset($wp_product_shipping_class_term[0]) && isset($wp_product_shipping_class_term[0]['term_id'])){
            	...
            	if (is_array($product_data[$this->product_field_shipping_class]) && !empty($product_data[$this->product_field_shipping_class])){
                    ...
                    } elseif (!is_array($product_data[$this->product_field_shipping_class]) && $product_data[$this->product_field_shipping_class] != ''){
        		...
        		if ($wp_product_shipping_class != $sl_id_product_shipping_class){
                    ...
                    } elseif ($sl_id_product_shipping_class != ''){
        ...
        if (isset($product_data[$this->product_field_manage_stock])){
            ...
            if (is_array($sl_product_manage_stock) && !empty($sl_product_manage_stock)){
                ...
                } elseif (!is_array($sl_product_manage_stock) && $sl_product_manage_stock !== '' && !is_bool($sl_product_manage_stock)){
                ...
                if ($sl_manage_stock == 'no'){
                    ...
                    if (isset($product_data[$this->product_field_stock_status])){
                        ...
                        if (is_array($sl_product_stock_status)){
                            ...
                            } elseif (!is_array($sl_product_stock_status) && $sl_product_stock_status !== ''){
                ...
                if (is_numeric($sl_stock) && (!$sl_stock_not_num || ($sl_stock_not_num && $sl_stock !== 0))){
                    ...
                    if (!isset($wp_product['_stock']) || (isset($wp_product['_stock']) && $wp_product['_stock'] != $sl_stock)){
                        ...
                        if ((!isset($wp_product['_stock_status']) || (isset($wp_product['_stock_status']) && $wp_product['_stock_status'] == 'outofstock')) && $sl_stock > 0){
                            ...
                            } elseif ((!isset($wp_product['_stock_status']) || (isset($wp_product['_stock_status']) && $wp_product['_stock_status'] == 'instock')) && $sl_stock <= 0){
        ...
        foreach ($linked_fields as $field_sales => $linked_type) {
            ...
            if (isset($product_data[$field_sales])){
                ...
                } elseif (!is_array($product_data[$field_sales]) && $product_data[$field_sales] != ''){
        ...
        sl_debug('## time_product_core_data: '.(microtime(true) - $time_ini_product_core_data).' seconds.', 'timer');

        $time_ini_product_images = microtime(true);
        //Product images
        if (!empty($product_data[$this->product_field_image])){
            ...
            if(count($sl_product_images) > 0) {
                ...
                if ($wp_product_image_gallery != ''){
                    ...
                    // $time_ini_read_image_gallery_data = microtime(true);
                    ...
                    // sl_debug('# time_read_image_gallery_data: '.(microtime(true) - $time_ini_read_image_gallery_data).' seconds.', 'timer');
                ...
                foreach ($this->product_images_sizes as $img_format) {
                    ...
                    // $time_ini_image_size = microtime(true);

                    foreach ($sl_product_images as $sl_product_image) {
                        
                        // $time_ini_product_image = microtime(true);
                        if (!empty($sl_product_image[$img_format])){
                            ...
                            if ($sl_product_image == $main_image){
                                ...
                                // $time_ini_process_main_image = microtime(true);
                                ...
                                // sl_debug('# time_process_main_image: '.(microtime(true) - $time_ini_process_main_image).' seconds.', 'timer');

                            }else{
                                
                                // $time_ini_process_image_gallery = microtime(true);
                                ...
                                // sl_debug('# time_process_image_gallery: '.(microtime(true) - $time_ini_process_image_gallery).' seconds.', 'timer');

                            }

                        }
                        // sl_debug('# time_product_image: '.(microtime(true) - $time_ini_product_image).' seconds.', 'timer');

                    }

                    // sl_debug('# time_image_size '.$img_format.': '.(microtime(true) - $time_ini_image_size).' seconds.', 'timer');
                }
            ...
            if (!isset($wp_product['_thumbnail_id']) || (isset($wp_product['_thumbnail_id']) && ((!is_array($wp_product['_thumbnail_id']) && $wp_product['_thumbnail_id'] != $wp_thumbnail_id) || is_array($wp_product['_thumbnail_id']) && $wp_product['_thumbnail_id'][0] != $wp_thumbnail_id))){
                ...
                // $time_ini_update_thumbnail_id = microtime(true);
                sl_update_post_meta( $wp_product['ID'], '_thumbnail_id', $wp_thumbnail_id );
                // sl_debug('# time_update_thumbnail_id: '.(microtime(true) - $time_ini_update_thumbnail_id).' seconds.', 'timer');
            ...
            if (!isset($wp_product['_product_image_gallery']) || (isset($wp_product['_product_image_gallery']) && $wp_product['_product_image_gallery'] != $wp_gallery_ids)){
                ...
                // $time_ini_update_product_image_gallery = microtime(true);
                sl_update_post_meta( $wp_product['ID'], '_product_image_gallery', $wp_gallery_ids );
                // sl_debug('# time_update_product_image_gallery: '.(microtime(true) - $time_ini_update_product_image_gallery).' seconds.', 'timer');
            ...
            if (!empty($wp_product_image_gallery_ids)){
                // $time_ini_delete_images = microtime(true);
                ...
                // sl_debug('# time_delete_images: '.(microtime(true) - $time_ini_delete_images).' seconds.', 'timer');

            }

        }
        sl_debug('## time_product_images: '.(microtime(true) - $time_ini_product_images).' seconds.', 'timer');
        ...
        foreach ($all_linked_product_data as $wp_product_id => $linked_product_data) {
            ...
            foreach ($linked_product_data as $linked_idx => $linked_data) {
                ...
                if ($linked_type == '_children'){
                    ...
                    } elseif (empty($linked_product_data) && $wp_product_type == 'grouped'){
    ...
    /**
     * Synchronize product attributes.
    ...
    public function find_product_by_sku($product_sku, $product_id, $comp_id)
    {
        ...
        if( is_wp_error( $posts ) ) {
            ...
            if (!empty($posts)){
                ...
                } elseif ($wp_saleslayerid == $product_id && $wp_saleslayercompid == $comp_id){
