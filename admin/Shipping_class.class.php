...
class Shipping_class
{
    ...
    /**
     * Load existing shipping classes
     * @return void
     */
    private function load_shipping_classes()
    {
        ...
        if (taxonomy_exists('product_shipping_class')) {
        	...
        	foreach ($shipping_classes_terms as $shipping_class_term) {
                ...
                $this->shipping_classes[$shipping_class_vars['term_id']] = array(
                    'name' => $shipping_class_vars['name'],
                    'slug' => $shipping_class_vars['slug']
                );
    ...
    public function create_shipping_class($shipping_class_name)
    {
        ...
        if (is_wp_error($new_shipping_class)) {
            ...
            sl_debug('## Error. Creating new shipping class with name '.$shipping_class_name.' : '.print_r($new_shipping_class->get_error_message(), true));
            return '';

        } else {
            
            $new_shipping_class_data = sl_get_term_by('term_id', $new_shipping_class['term_id'], 'product_shipping_class', 'ARRAY_A');
            $this->shipping_classes[$new_shipping_class_data['term_id']] = array(
                'name' => $new_shipping_class_data['name'],
                'slug' => $new_shipping_class_data['slug']
            );
