...
class Synchronize
{
    ...
    /**
     * Get data schema from the connector images.
    ...
    private function get_data_schema($slconn)
    {
        ...
        if ($this->debug_level > 2) sl_debug('Schema: '.print_r($schema, true));
    ...
    private function check_process_time()
    {
        ...
        $current_process_time = microtime(true) - $this->sl_time_ini_sync_data_process;
    ...
    /**
     * Initialize catalogue vars to load before synchronizing.
     * @return void
     */
    private function initialize_vars()
    {
        ...
        if (!$this->initialized_vars){
                ...
                $this->category_fields = array(
                    ...
                    'category_field_description_short',
                ...
                $this->product_format_fields = array(
                    'format_field_sku',
    ...
    /**
     * Delete queued syncdata rows when the batch size limit is reached.
     *
     * @param bool $force_delete Whether to force deletion even if the batch size is below the threshold.
     * @return void
     */
    private function check_sql_items_delete(bool $force_delete = false)
    {
    ...
    /**
     * Check sync data pid flag in database and delete kill it if the process is stuck.
     * @return void
     */
            ...
            if (!empty($current_flag) && isset($current_flag[0])){
                ...
                if ($current_flag['syncdata_pid'] == 0){
                    ...
                    if ($minutes < 10){
                        ...
                        sl_debug('## Error. Exception killing pid '.$current_flag['syncdata_pid'].': '.print_r($e->getMessage(), true), 'syncdata');
    ...
    /**
    * Disable sync data pid flag in database.
    * @return void
    */
    ...
    /**
     * Load syncdata counters into a class variable.
     * @return void
     */
    private function load_syncdata_counters()
    {
        ...
        if (!empty($counters_info) && isset($counters_info[0])){
            ...
            $this->counters_info['info'] = json_decode(stripslashes($counters_info[0]['item_data']), true);
    ...
    /**
     * Update syncdata counters in database.
    ...
    /**
     * Synchronize Sales Layer connectors data stored in sync data table.
     * @return void
     */
    public function sync_data_connectors()
    {
        ...
        $this->sl_time_ini_sync_data_process = microtime(true);
        ...
        sl_debug('### time_all_syncdata_process: '.(microtime(true) - $this->sl_time_ini_sync_data_process).' seconds.', 'syncdata');
    ...
    public function store_sync_data($connector_id, $secret_key)
    {
        ...
        $time_ini_all_process = microtime(true);
        ...
        $pagination = $general_params->getInfo('pagination');
            ...
            $slconn->set_pagination($pagination);
                $debug_pagination_text = ', Pagination: '.print_r($pagination, true);
        }
        ...
        $time_ini_all_store_process = microtime(true);
        ...
        do {	
            ...
            if ($slconn->have_next_page() && $slconn->get_next_page_info()) $is_next_page = true;				
                sl_debug('Page: '.print_r($page, true).' - Is_next_page:'.print_r($is_next_page, true));
            ...
            if ($this->checkIfResponseDataHasData($pagination_response_data)){
                ...
                foreach ($pagination_response_data as $nombre_tabla => $data_tabla) {
                        ...
                        if (count($deleted_data) > 0) {
                            ...
                            switch ($nombre_tabla) {
                                case 'catalogue':
                                    ...
                                    if ($this->debug_level > 1) sl_debug('Delete categories data to store: '.print_r($deleted_data, true));
                                ...
                                sl_debug('#### time_store_items_delete - '.$item_type.': '.(microtime(true) - $time_ini_store_items_delete).' seconds.');
                        ...
                        switch ($nombre_tabla) {
                            case 'catalogue':
                                ...
                                if (($modified_data = $this->storeCategoriesPaginated($pagination_response_data, $is_next_page)) !== false){
                                    ...
                                    if ($this->debug_level > 1) sl_debug('Sync categories data to store: '.print_r($modified_data, true));
                            ...
                            case 'product_formats':
                                ...
                                if ($this->debug_level > 1) sl_debug('Product formats data: '.print_r($modified_data, true));
                    ...
                    sl_debug('#### time_store_items_update - '.$item_type.': '.(microtime(true) - $time_ini_store_items_update).' seconds.');
        ...
        sl_debug('##### time_all_store_process: '.(microtime(true) - $time_ini_all_store_process).' seconds.');
    ...
    /**
     * Run through stored indexes and return div messages to show after store data
    ...
    /**
     * Check the the response data has modified or deleted items
    ...
    /**
     * Store categories in case the next page has more categories, so the reorganization function works with all categories
    ...
    /**
     * Insert sync data into the database.
